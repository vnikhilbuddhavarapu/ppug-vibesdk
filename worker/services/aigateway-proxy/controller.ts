...
import { apps, users } from '../../database/schema';
...
function countImageBytes(messages: unknown): number {
	...
	for (const message of messages) {
		...
		for (const part of content) {
			const url = (part as { image_url?: { url?: unknown } } | null)
				?.image_url?.url;
...
export async function proxyToAiGateway(
	...
	console.log(
		`[AI Proxy] Received request: ${request.method} ${request.url}`,
	);
	if (!env.AI_PROXY_JWT_SECRET) {
		...
		return new Response(
			JSON.stringify({
				error: {
					message:
						'AI Gateway proxy is not enabled for this platform',
					type: 'invalid_request_error',
				},
			}),
			{
	...
	try {
		...
		console.log(
			`[AI Proxy] Authenticated request from app: ${app.title} (${app.id}), user: ${app.userId} (${owner?.email})`,
		);
		...
		if (!ALLOWED_PROXY_PATHS.has(targetPath)) {
			return jsonError(
				404,
				`Unsupported proxy path: ${targetPath}`,
				'invalid_request_error',
			);
		}
		...
		if (requestBody.messages !== undefined) {
			if (!Array.isArray(requestBody.messages)) {
				return jsonError(
					400,
					'Invalid parameter: messages must be an array',
					'invalid_request_error',
					{ param: 'messages' },
				);
			}
			...
			if (countImageBytes(requestBody.messages) > MAX_IMAGE_BYTES) {
				return jsonError(
					413,
					'Inline image payload too large',
					'invalid_request_error',
				);
			}
		}
