...
type RateLimitEnv = Parameters<
	typeof RateLimitService.enforceLLMCallsRateLimit
>[0];

function createConfig(
	overrides: Partial<RateLimitSettings['llmCalls']> = {},
): RateLimitSettings {
	return {
...
describe('RateLimitService.enforceLLMCallsRateLimit', () => {
	it('increments the user LLM credit bucket by the supplied credit cost', async () => {
		...
		expect(increment).toHaveBeenCalledWith(
			...
			expect.objectContaining({ limit: 10_000, period: 86_400 }),
			2,
		);
	});
	...
	it('does not throw when an increment exceeds the limit in metering mode', async () => {
		const increment = vi
			.fn()
			.mockResolvedValue({ success: false, exceededLimit: 'daily' });
	...
	it.each([
		...
		[
			'Cloudflare-connected exclusion',
			createConfig({ excludeCloudflareConnected: true }),
			false,
			true,
			2,
		],
	] as const)(
		'does not increment when %s applies',
		async (
			_reason,
			config,
			isUsingBYOK,
...
describe('RateLimitService.getRequestIdentifier', () => {
	...
	it('returns the same identifier for the same IP with different token cookies', async () => {
		const id1 = await RateLimitService.getRequestIdentifier(
			request({
				'CF-Connecting-IP': '1.2.3.4',
				Cookie: 'accessToken=aaaa',
			}),
		);
		const id2 = await RateLimitService.getRequestIdentifier(
			request({
				'CF-Connecting-IP': '1.2.3.4',
				Cookie: 'accessToken=bbbb',
			}),
		);
	...
	it('returns different identifiers for different IPs', async () => {
		const id1 = await RateLimitService.getRequestIdentifier(
			request({ 'CF-Connecting-IP': '1.2.3.4' }),
		);
		const id2 = await RateLimitService.getRequestIdentifier(
			request({ 'CF-Connecting-IP': '5.6.7.8' }),
		);
	...
	it('falls back to ip:unknown when no IP headers are present', async () => {
		const id = await RateLimitService.getRequestIdentifier(
			request({ Authorization: 'Bearer aaaa' }),
		);
