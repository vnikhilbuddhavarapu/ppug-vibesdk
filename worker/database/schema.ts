...
const PROVISIONED_RESOURCE_TYPE_VALUES = [
	'r2',
	'd1',
	'kv',
	'vectorize',
] as const;
const PROVISIONED_RESOURCE_STATUS_VALUES = ['active', 'deleted'] as const;
 ...
 */
export const userOauthIdentities = sqliteTable(
	...
	(table) => ({
		providerIdx: uniqueIndex(
			'user_oauth_identities_provider_unique_idx',
...
export const appViews = sqliteTable(
	...
	(table) => ({
		...
		appViewerIdx: uniqueIndex('app_views_app_viewer_idx').on(
			table.appId,
			table.viewerHash,
		),
	}),
...
export const cloudflareAccounts = sqliteTable(
	...
	(table) => ({
		...
		accountIdIdx: index('cloudflare_accounts_account_id_idx').on(
			table.accountId,
		),
		userAccountIdx: uniqueIndex('cloudflare_accounts_user_account_idx').on(
			table.userId,
			table.accountId,
		),
	}),
 ...
 */
export const aiGateways = sqliteTable(
	'ai_gateways',
	{
		id: text('id').primaryKey(),
		userId: text('user_id')
		...
		cloudflareAccountId: text('cloudflare_account_id')
			.notNull()
			.references(() => cloudflareAccounts.id, { onDelete: 'cascade' }),
		...
		creditsLastUpdated: integer('credits_last_updated', {
			mode: 'timestamp',
		}),

		// Configuration
		autoCreated: integer('auto_created', { mode: 'boolean' }).default(
			false,
		), // Was this auto-created?
	...
	(table) => ({
		...
		accountIdx: index('ai_gateways_account_idx').on(
			table.cloudflareAccountId,
		),
		userAccountIdx: index('ai_gateways_user_account_idx').on(
			table.userId,
			table.cloudflareAccountId,
		),
		gatewayIdIdx: uniqueIndex('ai_gateways_gateway_id_idx').on(
			table.cloudflareAccountId,
			table.gatewayId,
		),
	}),
...
/**
 * Provisioned Resources table - Cloudflare resources (R2/D1/KV/Vectorize)
 * provisioned on demand by Think apps. Tracks ownership for per-user caps,
 * audit, and cleanup, and the conventional binding name the deployer wires
 * into the generated worker's bindings.
 */
export const provisionedResources = sqliteTable(
	'provisioned_resources',
	{
		...
		resourceType: text('resource_type', {
			enum: PROVISIONED_RESOURCE_TYPE_VALUES,
		}).notNull(),
		resourceName: text('resource_name').notNull(), // City-scoped CF-side name, e.g. ppug-mtl-<hash>-<appId>-r2
		resourceId: text('resource_id').notNull(), // CF-assigned id (KV/D1/Vectorize) or bucket name (R2)
		bindingName: text('binding_name').notNull(), // Conventional binding name injected into the worker, e.g. BUCKET/DB/KV/VECTORIZE

		// Status and metadata
		status: text('status', { enum: PROVISIONED_RESOURCE_STATUS_VALUES })
			.notNull()
			.default('active'),
		createdAt: integer('created_at', { mode: 'timestamp' }).default(
			sql`CURRENT_TIMESTAMP`,
	...
	(table) => ({
		userTypeIdx: index('provisioned_resources_user_type_idx').on(
			table.userId,
			table.resourceType,
		),
		appIdx: index('provisioned_resources_app_idx').on(table.appId),
		appBindingIdx: uniqueIndex('provisioned_resources_app_binding_idx').on(
			table.appId,
			table.bindingName,
		),
	}),
...
export type ProvisionedResource = typeof provisionedResources.$inferSelect;
export type NewProvisionedResource = typeof provisionedResources.$inferInsert;
