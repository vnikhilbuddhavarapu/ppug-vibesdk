...
export class AuthService extends BaseService {
	...
	async getOauthProvider(
		...
		switch (provider) {
			...
			case 'cloudflare':
				return CloudflareConnectOAuthProvider.createForLogin(
					this.env,
					url,
				);
			default:
	...
	async getOAuthAuthorizationUrl(
		...
		if (intendedRedirectUrl) {
			validatedRedirectUrl = validateRedirectUrl(
				intendedRedirectUrl,
				request,
			);
		}
	...
	async handleOAuthCallback(
		...
		try {
			...
			const oauthState = await this.validateAndConsumeOAuthState(
				provider,
				state,
				request,
			);
	...
	/**
	 * JIT-provision and log in a user who has already been authenticated by
	 * Cloudflare Access (the caller has verified the `CF_Authorization` JWT).
	 * Reuses the same identity-first find-or-create path as every other OAuth
	 * provider so cost attribution, resource naming, and account linking treat
	 * an Access login identically to GitHub/Google/Cloudflare.
	 */
	async handleAccessLogin(
		...
		try {
			// Deployment-level admission gate (ALLOWED_EMAIL). Enforced BEFORE any
			// user row or session is created so a rejected email never gets admitted.
			enforceAllowedEmail(this.env, identity.email, 'oauth');

			const oauthUserInfo: OAuthUserInfo = {
				id: identity.sub,
				email: identity.email,
				name: identity.name,
				// Access's identity provider (Cloudflare Dashboard) has already
				// verified this identity before minting the JWT we validated.
				emailVerified: true,
			};
			...
			logger.info('Access login successful', { userId: user.id });
			...
			logger.error('Access login error', error);
			throw new SecurityError(
				SecurityErrorType.UNAUTHORIZED,
	...
	async getUserIdentities(
		userId: string,
	): Promise<schema.UserOauthIdentity[]> {
	...
	async getUserForAuth(userId: string): Promise<AuthUser | null> {
		try {
			...
			if (!user) {
				logger.debug('User not found or not eligible for auth', {
					userId,
				});
				return null;
			}
