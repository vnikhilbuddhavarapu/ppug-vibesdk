...
import {
	...
	verifyAccessJwt,
} from '../../../middleware/auth/accessAuth';
/**
...
export class AuthController extends BaseController {
	...
	static async logout(
		...
		// Access-originated sessions additionally need the Access hosted logout
		// URL so the browser drops its `CF_Authorization` cookie too; otherwise
		// "Continue with Access" silently re-authenticates on the next click.
		const logoutUrl =
			routeContext.user?.provider === 'access'
				? (buildAccessLogoutUrl(env) ?? undefined)
				: undefined;
	...
	static async initiateOAuth(
		...
		try {
			...
			const authService = new AuthService(env);
			const { authUrl, nonce } =
				await authService.getOAuthAuthorizationUrl(
					validatedProvider,
					request,
					intendedRedirectUrl,
				);
	...
	static async handleOAuthCallback(
		...
		try {
			...
			const linkUserId = await authService.getPendingLinkUserId(
				validatedProvider,
				state,
			);
			if (linkUserId) {
				...
				if (!session || session.user.id !== linkUserId) {
					return Response.redirect(
						`${baseOrigin}/settings?error=link_unauthorized`,
						302,
					);
				}
				...
				try {
					...
					const location =
						safeLinkRedirect ||
						`${baseOrigin}/settings?linked=${validatedProvider}`;
					...
					response.headers.append(
						'Set-Cookie',
						buildClearOAuthNonceCookie(env),
					);
					return response;
				} catch (error) {
					...
					const reason =
						...
						error.statusCode === 409
							...
							: 'link_failed';
					return Response.redirect(
						`${baseOrigin}/settings?error=${reason}`,
						302,
					);
				}
			}
			...
			const baseUrl = new URL(request.url).origin;
			const reason =
				error instanceof SecurityError && error.statusCode === 409
					? 'email_exists'
					: 'auth_failed';
	...
	static async createApiKey(
		...
		try {
			...
			const apiKeyService = new ApiKeyService(env);
			const activeKeyCount = await apiKeyService.getActiveApiKeyCount(
				user.id,
			);
	...
	static async getAuthProviders(
		...
		try {
			const providers = {
				...
				access: isAccessLoginEnabled(env),
				email: isEmailAuthEnabled(env),
			};
			const hasOAuth =
				providers.google || providers.github || providers.cloudflare;
	...
	/**
	 * Handle the Cloudflare Access login callback.
	 *
	 * Access protects this exact route (`/auth/access/callback`, see
	 * `worker/app.ts`) and only forwards the request here once its own hosted
	 * login has succeeded, attaching a signed `CF_Authorization` JWT. This
	 * verifies that JWT, JIT-provisions/logs the user in through the existing
	 * `AuthService`, and redirects into the app with the normal session cookie.
	 * GET /auth/access/callback
	 */
	static async handleAccessCallback(
		request: Request,
		env: Env,
		_ctx: ExecutionContext,
		routeContext: RouteContext,
		...
		try {
			...
			const authService = new AuthService(env);
			const result = await authService.handleAccessLogin(
				identity,
				request,
			);

			const intendedRedirect =
				routeContext.queryParams.get('redirect_url');
			const safeRedirect = intendedRedirect
				? validateRedirectUrl(intendedRedirect, request)
				: null;
			const redirectLocation = safeRedirect || `${baseUrl}/`;
			...
			this.logger.error('Access login failed', error);
			return Response.redirect(`${baseUrl}/?error=access_failed`, 302);
		}
	}
	...
	private static async autoConnectCloudflareGateway(
		...
		try {
			...
			response.headers.append(
				'Set-Cookie',
				buildTokenCookie(env, encryptedBlob),
			);

			this.logger.info('Cloudflare login auto-connect succeeded', {
				userId,
			});
		} catch (error) {
			// Identity login must still succeed; the user falls back to free tier + Connect.
			this.logger.warn(
				'Cloudflare login auto-connect failed (continuing with identity login)',
				{
					userId,
					error:
