...
import { AI_MODEL_CONFIG } from '../../../agents/inferutils/config.types';
...
export class ModelConfigController extends BaseController {
	...
	static async updateModelConfig(
		...
		try {
			...
			if (error instanceof z.ZodError) {
				return ModelConfigController.createErrorResponse<ModelConfigUpdateData>(
					'Validation failed: ' + JSON.stringify(error.issues),
					400,
				);
			}
	...
	static async getByokProviders(
		...
		try {
			...
			const validAgentActions = Object.keys(
				AGENT_CONFIG,
			) as AgentActionKey[];
			if (
				agentActionParam &&
			...
			if (agentAction) {
				...
				const allAccessibleModels =
					Object.values(allModelsByProvider).flat();
				...
				const responseData: ByokProvidersData = {
					...
					modelLabels: ModelConfigController.buildModelLabels(),
				};

				return ModelConfigController.createSuccessResponse(
					responseData,
				);
			}
	...
	/** Friendly display name per model id, e.g. "workers-ai/@cf/zai-org/glm-5.3" -> "GLM 5.3". */
	private static buildModelLabels(): Record<string, string> {
	 ...
	 * constraint), for the Settings model-config picker. Reuses the same
	 * ModelConfigService method the WebSocket `get_model_configs` handler uses.
	 * GET /api/model-configs/info
	 */
	static async getModelConfigsInfo(
		_request: Request,
		env: Env,
		_ctx: ExecutionContext,
		context: RouteContext,
	): Promise<ControllerResponse<ApiResponse<ModelConfigsInfoData>>> {
		try {
			const user = context.user!;
			const modelConfigService = new ModelConfigService(env);
			const info = await modelConfigService.getModelConfigsInfo(user.id);

			const responseData: ModelConfigsInfoData = {
				...info,
				message: 'Model configurations retrieved successfully',
			};
			...
			this.logger.error(
				'Error getting model configurations info:',
				error,
			);
			return ModelConfigController.createErrorResponse<ModelConfigsInfoData>(
				'Failed to get model configurations',
				500,
			);
		}
