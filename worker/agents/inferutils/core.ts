...
function buildJsonSchemaResponseFormat(
	...
	return {
		...
		json_schema: {
			name,
			strict: true,
			schema: sanitizeStrictJsonSchema(jsonSchema) as Record<
				string,
				unknown
			>,
		},
...
export async function buildGatewayUrl(
	...
	if (gatewayOverride?.baseUrl) {
		...
		if (url.protocol !== 'https:') {
			throw new Error(
				`Invalid AI gateway baseUrl: only https is allowed (got ${url.protocol})`,
			);
		}
	...
	if (
		...
		try {
			...
			if (url.protocol === 'http:' || url.protocol === 'https:') {
				...
				url.pathname = buildGatewayPathname(
					cleanPathname,
					providerOverride,
				);
				return url.toString();
...
function resolvePlatformGatewayToken(
	...
	return (
		...
		(env.CLOUDFLARE_AI_GATEWAY_TOKEN || env.CLOUDFLARE_API_TOKEN)
	);
}
...
async function getApiKey(
	...
	if (shouldUseUserKey) {
		logger.info(
			'User exceeded free tier limits, attempting to use user API token',
		);
		...
		if (encryptedUserToken) {
			try {
				const accessToken = await getAccessTokenFromBlob(
					encryptedUserToken,
					env,
					userId,
				);
				if (accessToken && isValidApiKey(accessToken)) {
					logger.info(
						'Using user decrypted Cloudflare AI Gateway token',
					);
		...
		logger.warn(
			'User exceeded limits but no valid API token found, using free tier anyway',
		);
	}
	...
	return {
		apiKey: resolvePlatformGatewayToken(
			env,
			runtimeOverrides?.aiGatewayOverride,
			false,
		),
...
export async function getConfigurationForModel(
		...
		switch (modelConfig.provider) {
			...
			default:
				providerForcedOverride =
					modelConfig.provider as AIGatewayProviders;
				break;
		}
	}
	...
	const { apiKey, isUserCredential } = await getApiKey(
	...
	if (requestedCustomGateway && !honorGatewayOverride) {
		logger.warn(
			'Ignoring user-supplied AI gateway baseUrl: resolved API key is platform-owned',
			{
	...
	const gatewayOverride = honorGatewayOverride
		? runtimeOverrides?.aiGatewayOverride
		: undefined;
	...
	const baseURL = await buildGatewayUrl(
		...
		providerForcedOverride,
		gatewayOverride,
		useUserGateway ? userGateway : null,
	);

	const gatewayToken = resolvePlatformGatewayToken(
		env,
		gatewayOverride,
	...
	const defaultHeaders =
		!useUserGateway && gatewayToken && apiKey !== gatewayToken
			? {
...
async function executeToolCalls(
	...
	return executeToolCallsWithDependencies(
		openAiToolCalls,
		originalDefinitions,
	);
}
...
function updateToolCallContext(
	...
	if (completionDetector && !completionSignal) {
		completionSignal =
			completionDetector.detectCompletion(executedToolCalls);
	}
...
export async function infer<OutputSchema extends z.ZodObject>(
	...
	try {
		...
		const hasCfConnected = await hasCloudflareConfigured(
			env,
			metadata.userId,
		);
		...
		try {
			...
			response = await client.chat.completions.create(
				...
				},
				{
					signal: abortSignal,
					headers: {
						'cf-aig-metadata': buildAigMetadataHeader({
							userId: metadata.userId,
							agentId: metadata.agentId,
							surface: actionKey === 'think' ? 'think' : 'phasic',
							actionKey,
							schemaName,
						}),
					},
